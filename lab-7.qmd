...
capitals_with_passes <- capitals_with_passes |> 
  ...
  #map_chr takes in a list, pass_times as its first argument, and a function as its second
  #The function returns the # element if we have one, and NA otherwise
  #Technically the "check" is not necessary, but it's safer to keep it
  select(-pass_times)
```
